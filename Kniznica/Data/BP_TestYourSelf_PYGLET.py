#Test yourself: 'ORIG>TRAN or 'TRAN>ORIG'
# INPUT ##################################################
...
if mode == 'ORIG>TRAN':
  ...
  shuffle_list = list(range(block_count))     
  random.shuffle(shuffle_list)             # [<random permutation>]
  blocks_left = block_count

  for n in range(block_count):
    #TRAN blocks left displayed
    for i in range(blocks_left):
      block_number = shuffle_list[i]
      print(i,' ', blocks_TRAN[block_number].strip())
    # n-th question
    b1 = blocks_ORIG[n].strip()
    if text_prompt:
    ...
    while True:
      ans = input(' block#: ')
      if audio and ans.upper() == 'A':
        play_block(a1,a2)
        continue
      if not text_prompt and ans.upper() == 'T':
        print(frmt.format(b1))
        continue
      if ans.isnumeric():
        ans = int(ans)
        if ans >= 0 and ans < blocks_left:
          if shuffle_list[ans] == n :
            print('+')
            blocks_left -= 1
            del shuffle_list[ans]
            break
          else: print('-')
        else: print('-')
      else: print('-')
...
if mode == 'TRAN>ORIG':
  # print randomly original blocks
  shuffle_list = list(range(block_count))     
  random.shuffle(shuffle_list)             # [<random permutation>]
  blocks_left = block_count
  
  for n in range(block_count):
    #ORIG blocks left displayed
    for i in range(blocks_left):
      block_number = shuffle_list[i]
      print(i,' ', blocks_ORIG[block_number].strip())
    # n-th question
    ...
    while True:
      ...
      if ans.isnumeric():
        ans = int(ans)
        if ans >= 0 and ans < blocks_left:
          if shuffle_list[ans] == n :  # correct answer
            play_block(a1,a2)
            if not audio: print('+')
            blocks_left -= 1
            del shuffle_list[ans]
